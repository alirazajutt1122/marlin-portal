...
import { NegotiatedDealsComponent } from '../negotiated-deals';

@Component({
    selector: 'negotiated-equity-order',
    templateUrl:'./negotiated-equity-order.html',
})

export class NegotiatedEquityOrderComponent {
    ...
    @ViewChild('negotiated-deals' , {static : false}) newNegotiatedOrderAllMenu : NegotiatedDealsComponent;
 ...
 ;
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.sharedData || changes.sharedSide)
    {
   ...
   if(this.sharedSide === 'S')
    ...
    validateOrder(): boolean {
      ...
      if (this.cmbSymbol.text === '' || this.cmbSymbol.text === AppConstants.PLEASE_SELECT_STR) {
        ...
        return false;
      }  else if (!this.order.counterBroker || this.order.counterBroker.trim().length === 0) {
        this.statusMsg = "Counter broker is required";        
