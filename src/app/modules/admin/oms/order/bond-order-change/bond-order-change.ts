...
export class BondOrderChange implements OnInit, AfterViewInit {
    ...
    updateOrder(orderResponse): void {
      ...
      if (this.isBondPricingMechanismPercentage)
        this.order.value = (this.changeOrder.volume * this.changeOrder.price_) / 100;
      else
        this.order.value = this.changeOrder.volume * this.changeOrder.price_;

      if (!this.isBondPricingModelDirty)
        this.order.settlementValue = Number(this.order.value) + Number(this.securityMarketDetails.accrudeProfit);
      else
        this.order.settlementValue = Number(this.order.value);
    }
    ...
    onVolumeChange(): void {
      if (this.isBondPricingMechanismPercentage)
        this.order.value = (Number(this.inputVolume.value) * Number(this.inputPrice.value)) / 100;
      else
