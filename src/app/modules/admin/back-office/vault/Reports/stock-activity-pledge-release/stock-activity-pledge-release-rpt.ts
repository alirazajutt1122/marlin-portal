...
@Component({
  ...
  selector: 'stock-activity-pledge-release-rpt',
  templateUrl: './stock-activity-pledge-release-rpt.html',
  encapsulation: ViewEncapsulation.None,
})
export class StockActivityPledgeReleaseRpt implements OnInit {
  ...
  pdfSrc: String
  pdf = false
  fileNameForDownload = "StockActivityPledgeReleaseReport.pdf"
 ...
 *********************************/
  initForm() {
    ...
    this.reportTypeList = this.params.getReportFormatOptionsForPledgeRelease();
    this.clearFields();
  }
  public clearFields() {
  ...
  public printReport(model: any, isValid: boolean) {
    ...
    if (isValid) {
      ...
      switch (this.reportType.toLowerCase()) {
        case 'com relacao ao cliente':
          rptType = 'client-wise'
          break;
        case 'seguranca':
          rptType = 'security-wise'
          break;
        case 'client wise':
          rptType = 'client-wise'
          break;
      ...
      this.reportService.getStockActivityPledgeRelease(this.params.PARTICIPANT_ID, this.params.SECURITY_ID, security, startDate, encodeURIComponent(this.params.FROM_ACCOUNT.valueOf()),
        encodeURIComponent(this.params.TO_ACCOUNT.valueOf()), this.params.EXCHANGE_ID, exchange, rptType, this.lang)
        .subscribe(
          restData => {
            this.loader.hide();
            var reportData: any;
