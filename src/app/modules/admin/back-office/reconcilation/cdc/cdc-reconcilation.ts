...
@Component({
  selector: 'cdc-reconcilation',
  templateUrl: './cdc-reconcilation.html',
})

export class CdcReconcilation implements OnInit {
  ...
  public btnUpload(model: any, isValid: boolean) {
    ...
    this.listingService.uploadCDCReconcilation(this.oCDCMigrator).subscribe(
      data => {
        this.loader.hide();
  ...
  exportToPdf() {
    ...
    wjcGridPDF.FlexGridPdfConverter.export(this.flexDetail, 'CDC-Reconcilation.pdf', { scaleMode: wjcGridPDF.ScaleMode.ActualSize });
