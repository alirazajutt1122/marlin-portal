...
@Component({
    selector: 'negotiated-bond-order',
    templateUrl:'./negotiated-bond-order.html',
})

export class NegotiatedBondOrderComponent {
    ...
    constructor(private appState: AppState, public authService: AuthService2,  public authServiceOMS : AuthService, private dataService: DataServiceOMS,
      ...
      private _fb: FormBuilder, private translate: TranslateService, public appUtility2: AppUtility, public socket : WebSocketService, public shareOrderService : ShareOrderService) {
    ...
    onClose() {
      this.init();
      this.loadSelectedSymbolFromMarketWatch();
      (jQuery('#new-negotiated-order-all-menu') as any).modal('hide');
