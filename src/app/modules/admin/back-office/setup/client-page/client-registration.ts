...
public onSaveDepository = () => {
    ...
    this.selectedDepositoryItem.Identification_Type_Id = AppConstants.DEFAULT_IDENTIFICATION_TYPE_ID
    this.selectedDepositoryItem.Identification_Number = this.selectedItem.contactDetail.taxNumber;
    ...
    if (!(AppUtility.isEmpty(this.itemsJointAccountList) || this.itemsJointAccountList.itemCount == 0)) {
      ...
      for (let i = 0; i < this.itemsJointAccountList.items.length; i++) {
        ...
        this.selectedDepositoryItem.JointAccountDetails[i].Name = this.itemsJointAccountList.items[i].contactDetail.fullName;
      ...
      this.listingService.getClientById(AppConstants.participantId, invClientId).subscribe(
        restData => {
          ...
          if (AppUtility.isValidVariable(restData.clientId)) {
            ...
            this.populateClientDocuemntsGrid();
           debugger
            if(AppUtility.isValidVariable(restData.contactDetail.firstName) && AppUtility.isValidVariable(restData.contactDetail.lastName)) {
            ...
            if(restData.statusCode === AppConstants.INV_STATUS_REGISTERED || restData.statusCode === AppConstants.INV_STATUS_ACTIVE) {
               this.isDisabledFields = true;
            }


            if(restData.statusCode === AppConstants.INV_STATUS_ACTIVE) {
...
if(this.itemsJointAccountList.items.length > 0 && this.showJoint && this.jointAccountGrid !== undefined) {
  if(AppUtility.isValidVariable(this.jointAccountGrid)){
    this.jointSelectedIndex =  this.jointAccountGrid.selection.row;
...
public updateClientStatus = (clientIdForStatus : Number)=> {
   ...
   this.listingService.updateClientStatusCode(x).subscribe((restData) => {
                ...
                this.onGetClientByID(this.clientIdForAR);
   }, error => {
        this.splash.hide();
        // this.errorMessage = error;
        // this.dialogCmp.statusMsg = this.errorMessage;
...
public onSaveAction(model: any, isValid: boolean) {
  ...
  private populateParticipants() {
    ...
    this.listingService.getParticipantListByExchagne(AppConstants.exchangeId).subscribe(restData => {
      ...
      if (!AppUtility.isEmptyArray(this.participantsList)) {
        //this.selectedItem.participant.participantId = this.participantsList[0].participantId;
      }
    },
      error => { this.splash.hide(); this.errorMessage = <any>error.message });
  }
  ...
  private validateAllowedMarkets(): boolean {
    for (let i = 0; i < this.itemsAllowedMarketList.itemCount; i++) {
      for (let j = 0; j < this.itemsAllowedMarketList.items.length; j++) {
        if (this.itemsAllowedMarketList.items[i].market.selected == true) {
          return true;
        }
      }
    }
