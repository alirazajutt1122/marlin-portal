...
@Component({
    selector: '[order-new-nav]',
    templateUrl: './order-new-nav.html',
...
export class OrderNewNav implements OnInit, AfterViewInit, OnDestroy {
    ...
    updateSymbolList(data) {
      ...
      if (AppUtility.isValidVariable(data) && !AppUtility.isEmpty(data)) {
        for (let i = 0; i < data.length; i++) { 
          if ((data[i].marketTypeCode !== AppConstants.MARKET_TYPE_BOND) && ( data[i].marketTypeCode !== AppConstants.MARKET_TYPE_ETF_)) {
