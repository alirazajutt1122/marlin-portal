...
@Component({
    selector: 'working-orders',
    templateUrl: './working-orders.html',
    encapsulation: ViewEncapsulation.None,
})
export class WorkingOrders implements OnInit, AfterViewInit {
    ...
    @ViewChild('orderChangeWO', { static: false }) orderChangeWO: OrderChange;
    @ViewChild('orderCancelWO', { static: false }) orderCancelWO: OrderCancel;
    ...
    constructor(private appState: AppState, public authServiceOMS: AuthService, private orderSvc: OrderService,
        private listingSvc: ListingService, private _fb: FormBuilder, private translate: TranslateService, private splash: FuseLoaderScreenService, private toast: ToastrService,) {
        ...
        if (!AppUtility.isEmpty(orders)) {
            for (let i = 0; i < orders.length; i++) {
                if (((AppUtility.isEmpty(this.exchangeCode) || this.exchangeId === 0) || this.exchangeCode === orders[i].exchange) &&
                    ...
                    ((AppUtility.isEmpty(this.custodian)) || this.custodian === orders[i].custodian) &&
                    (AppUtility.isEmpty(this.account.trim()) || this.account === orders[i].account || this.account === orders[i].account)) {
                    let o: Order = new Order();
                    o.setOrder(orders[i]);
                    o.type_ = AppUtility.ucFirstLetter(o.type_);
                    o.state_time = <any>wjcCore.Globalize.formatDate(new Date(o.state_time), AppConstants.DATE_TIME_FORMAT);
                    ...
                    (AppUtility.isEmpty(this.symbol.trim()) || this.symbol === orders[i].symbol) &&
                    ((AppUtility.isEmpty(this.custodian)) || this.custodian === orders[i].custodian) &&
                    (AppUtility.isEmpty(this.account.trim()) || this.account === orders[i].account || this.account === AppConstants.ALL_STR)) {
                    let o: Order = new Order();
                    o.setOrder(orders[i]);
                    o.type_ = AppUtility.ucFirstLetter(o.type_);
                    o.state_time = <any>wjcCore.Globalize.formatDate(new Date(o.state_time), AppConstants.DATE_TIME_FORMAT);
    ...
    updateMarketData(markets) {
        ...
        for (let i = 0; i < markets.length; i++) {
            if (markets[i].marketType.description === AppConstants.MARKET_TYPE_EQUITY) {
    ...
    exportExcel() {
        wjcGridXlsx.FlexGridXlsxConverter.save(this.flexGrid, { includeColumnHeaders: this.includeColumnHeader, includeCellStyles: false }, 'Equities Working Orders.xlsx');
    }
    ...
    exportPDF() {
        gridPdf.FlexGridPdfConverter.export(this.flexGrid, 'Equities Working Orders' + new Date().toLocaleString() + '.pdf', {
            maxPages: 10,
