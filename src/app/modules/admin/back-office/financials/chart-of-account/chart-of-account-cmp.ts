 ...
 **************************************/
  ...
  updateData(nodes) {
    ...
    for (var i = 0; i < nodes.length; i += 1) {
      node = nodes[i];
      if (!AppUtility.isEmpty(node.dbSecpAccount)) {
        node.glCodeDisplayName_ = node.glDesc + ' - [' + node.glCode + ']' + ' - [' + node.dbSecpAccount.accountCode + ']';
      }
      else {
        node.glCodeDisplayName_ = node.glDesc + ' - [' + node.glCode + ']' + ' - [' + node.dbSecpAccount + ']';
      }
      node.children = [];
  ...
  private addFromValidations() {
    this.myForm = this._fb.group({
      ...
      ncb: [''],
      lcb: [''],
      dbSecondarycode: ['', Validators.compose([Validators.required])],
      dbSecphead: [''],
      crSecondarycode: ['', Validators.compose([Validators.required])],
      crSecphead: [''],
    });
  }
